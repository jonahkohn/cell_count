...
def readImg(img_name, path, channels, dimensions):
    ...
    img = load_img(os.path.join(path, img_name), grayscale = True, target_size = [dimensions, dimensions])
...
def label_mask(mask, threshold=0.8, min_pixel=40, do_watershed=False, exclude_border=False):
    if mask.ndim == 3:
    ...
    # label image regions
    # JT: maybe dont label untill image is put back together?
    ...
    if do_watershed:
        ...
        local_maxi = peak_local_max(distance, indices=False, exclude_border=exclude_border, #False,
